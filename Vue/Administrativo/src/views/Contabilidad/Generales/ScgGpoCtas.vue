    ...
    <div class="glass-wrapper">
        ...
        <div class="form-card">
            <div class="form-grid">
                <div>
                    ...
                    <input type="string" id="tipo_cuenta" v-model="formData.tipo_cuenta" :readonly="mode === 'view'" required />
                </div>
                <div>
            ...
            <div class="form-grid">
                ...
                <div>
                    <label for="nom_status">Estado</label>
                    <select id="nom_status" v-model="formData.nom_status" :disabled="mode === 'view'" required>
        ...
        <div class="detalle-table">
            ...
            <table>
                <thead>
                    <tr>
                        <th class="resizable">Grupo <span class="resizer" @mousedown="startResize($event)"
                ...
                <tbody>
                    <tr v-for="(fila, index) in registros" @click="cargaLinea(index)"  :key="index">
                        <td>
                           <input v-model="fila.grupo_cuenta" :readonly="mode === 'view'" required />  
                        </td>
                        <td>
...
import { Scggcta, Scgsgcta } from '@/interface/interfaces';
...
function focoHijo() {
    ...
    document.getElementById(`grupo_cuenta[${cantidad}]`)?.focus();
}
...
const getScgsgcta = ref<any[]>([]);
const getScggcta = async () => {
    ...
    try {
        ...
        const response = await axios.get(
            `${dUrl.urlGlobal}/api3/scgsgcta/filter?tipo_cuenta=${formData.value.tipo_cuenta}`
        );

        // Asignamos los datos a getCaratvued y a registros
        getScgsgcta.value = response.data; // Cargamos los datos en getCaratvued

        // Mapeamos los datos obtenidos para ajustarlos al formato de registros
        registros.value = getScgsgcta.value.map((i: any) => ({
            tipo_cuenta: i.tipo_cuenta || '',
            grupo_cuenta: i.grupo_cuenta || '',
...
async function handleInsert() {
    ...
    if (!formData.value.tipo_cuenta || !formData.value.descripcion || !formData.value.uso || !formData.value.status) {
        error('Complete todos los campos obligatorios');
        return;
    }

    try {
        const response = await axios.post(dUrl.urlGlobal + '/api3/query', { tabla: 'sgcgcta', filtro: { tipo_cuenta: formData.value.tipo_cuenta}});
        const existe = response.data.length > 0;
        if (!existe) {
            error('Tipo de cuenta ya existe...')
            return
        }
    }
    catch (err: any) {
    ...
    if (!valida(detallesActivos.map(r => r.grupo_cuenta))) {
    ...
    const dataCabecera = { model: "scggcta", data: { tipo_cuenta: formData.value.tipo_cuenta, 
    ...
    try {
        ...
        for (const fila of detallesActivos) {
            const dataDetalle = { model: "scgsgcta", data: { tipo_cuenta: formData.value.tipo_cuenta, 
...
const upsertDetalle = async (fila: { grupo_cuenta: string; descripcion: any; status: any }) => {
    if (!fila.grupo_cuenta) return;
    
    try {
        const response = await axios.post(dUrl.urlGlobal + '/api3/query', { tabla: 'sgcsgcta', filtro: { tipo_cuenta: formData.value.tipo_cuenta, grupo_cuenta: fila.grupo_cuenta} });
        const existe = response.data.length > 0;
        if (!existe) {
            const dataInsert = { tabla: 'scgsgcta', data: { ...fila, ...formData.value, creado_por: userStore.globalUser, fecha_creado: dateTimeStore.formattedDate, hora_creado: dateTimeStore.formattedTime } };
            ...
            if (!resInsert.ok) error(`Error insertando el grupo de cuenta ${fila.grupo_cuenta}`);
        } else {
            ...
            if (resUpdate.status !== 200) error(`Error actualizando el grupo ${fila.grupo_cuenta}`);
        }
    } catch (err: any) {
        error(err.response?.data?.detail || `Error procesando cargo ${fila.grupo_cuenta}`);
    }
};
...
const handleUpdate = async () => {
    ...
    try {
        const responseCab = await axios.post(dUrl.urlGlobal + '/api3/update/', {
            table: 'scggcta',
            filters: { tipo_cuenta: formData.value.tipo_cuenta },
            data: {
                tipo_cuenta: formData.value.uso,
                descripcion: formData.value.descripcion,
                status: formData.value.status,
                modificado_por: userStore.globalUser,
                fecha_status: dateTimeStore.formattedDate,
...
const handleDelete = async () => {
    ...
    if (registros.value.length > 0) {
        ...
        const pregunta = await axios.post(dUrl.urlGlobal + '/api3/query', { tabla: 'scgcata',
                            filtro: { tipo_cuenta: formData.value.tipo_cuenta,
                                      grupo_cuenta: fila_del.grupo_cuenta }})

        if (pregunta.data.lenght > 0) {
            error('Existen cuentas contables con este grupo... no pueden eliminarse')
            return
        }

        const result = await question(`Se va a eliminar el grupo de cuenta ${fila_del.grupo_cuenta}`, '¿Deseas eliminarla?');
        if (!result.isConfirmed) return;
        ...
        try {
            ...
            console.log(formData.value.tipo_cuenta)
            console.log(fila_del.grupo_cuenta)

            const delfila = await axios.post(dUrl.urlGlobal + '/api3/delete', { table: 'scgsgcta', filters: { tipo_cuenta: formData.value.tipo_cuenta, 
                                                                                                              grupo_cuenta: fila_del.grupo_cuenta } })

            console.log(delfila)
            if (delfila.status !== 200) {
                error(`No se pudo eliminar el Grupo de Cuenta ${fila_del.grupo_cuenta}`)
                return
            }
        }
        catch (err: any) {
    ...
    const resultado = await question(`Se va a eliminar el tipo de cuenta ${formData.value.tipo_cuenta}`, '¿Deseas eliminarla?');
    ...
    try {
        ...
        success(`Se eliminó el Tipo de Cuenta ${formData.value.tipo_cuenta}`, 'Evento exitoso')
...
const handleSearch = async () => {
    ...
    try {
        ...
        const response = await axios.post(dUrl.urlGlobal + '/api3/query', 
                        { tabla: 'scggcta', filtro: { tipo_cuenta: formData.value.tipo_cuenta,
                                                          descripcion: formData.value.descripcion,
                                                          uso: formData.value.uso,
                                                          status: formData.value.status} });
